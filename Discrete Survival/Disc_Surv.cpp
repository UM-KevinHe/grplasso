...
tuple<vec, vec, vec, double, int> Disc_Surv_fit(vec &delta_obs, int max_timepoint, mat &Z, vec &time, vec gamma, vec beta, vec eta, int K0, vec &K1,
  ...
  vec old_beta = beta, r(n_obs), r_shift, w(n_obs);
...
List Disc_Surv_lasso(vec &delta_obs, int max_timepoint, mat &Z, vec &time, vec &gamma, vec &beta, int K0, vec &K1, vec &sum_failure, 
  ...
  for (int l = 0; l < n_lambda; l++){
    ...
    auto fit = Disc_Surv_fit(delta_obs, max_timepoint, Z, time, gamma, beta, eta, K0, K1, sum_failure, lambda, tol_iter, max_total_iter, max_each_iter, penalized_multiplier, backtrack, MM, bound, tol, active_var, n_obs, n_var, threads, actSet, actIter, activeVarNum, actSetRemove);
