...
# ls <- list(m = 200, poisson.mean = 100, rho = rho, n.rep = 10000)

setwd("/home/ybshao/GroupLasso/Simulations/Figures/FE_vs_Pooled/From_Poisson")
sim2 <- function(ls){
  start.time <- Sys.time()
  sd.gamma <- 1
  m <- ls$m
  ...
  RMSE.beta <- sqrt(bias.beta ^ 2 + ESD.beta ^ 2)
  Absbias.beta <- rowMeans(abs(mat - beta_true))
  rst <- c(ls$m, ls$poisson.mean, rho, bias.beta, ESD.beta, RMSE.beta, Absbias.beta) 
