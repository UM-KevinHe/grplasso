...
set.lambda.pplasso <- function(Y, Z, ID, n.prov, gamma.prov, beta, penalty.factor,
...
newZG.pplasso <- function(data, Z.char, standardize){
  ...
  if (standardize == T) {
    ...
    scale.Z <- attributes(new.Z)$`scaled:scale`
    nz <- which(scale > 1e-6)
    std.Z <- new.Z[, nz, drop = F]
    res <- list(std.Z = std.Z, nz = nz, center = center.Z, scale = scale.Z)
  } else {
    scale <- apply(as.matrix(Z), 2, mysd)  
    nz <- which(scale > 1e-6) #remove constant columns
    std.Z <- new.Z[, nz, drop = F]
    res <- list(std.Z = std.Z, nz = nz)
  }
  return(res)
}
